...
function verificarPass(){
    ...
    $regexp_passworddeb = /^(?=.*[a-zA-z])(?=.*\d)[A-Za-z\d$]{0,5}/;
    $regexp_passwordnor = /^(?=.*[a-zA-z])(?=.*\d)[A-Za-z\d$]{7,}/;
    $regexp_passwordfue = /^(?=.*[a-zA-z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{7,}/;
    
    //var pass = $("#pass").val;
    var $pass = document.getElementById("pass").value;

    if ($regexp_passworddeb.test($pass)){
      $("#mensaje").text("Contraseña débil").css("color", "red");
    } else if ($regexp_passwordnor.test($pass)){
      $("#mensaje").text("Contraseña normal").css("color", "yellow");
    } if ($regexp_passwordfue.test($pass)){
      $("#mensaje").text("Contraseña fuerte").css("color", "green");
    }


}

function verificar2Pass(){
    var p1 = document.getElementById("pass").value;
    var p2 = document.getElementById("pass2").value;

    if (p1 != p2) {
      $('#mensaje2').text("Las contraseñas deben de coincidir.").css("color", "red");
      return false;
    } else {
      $('#mensaje2').text("Las contraseñas coinciden.").css("color", "green");
      return true; 
    }
...
function generarHash(){
  ...
  var $pordefecto = 9007199254740991;

  var $cadena = $dias+$descargas;
  if ($cadena!=""){
    $valorHash = md5($cadena);
  } else {
    $valorHash = $pordefecto;
  }

  document.getElementById("hash").value=$valorHash;
}
...
function seleccionarIcono(){
      var $nombre = document.getElementById('archivo');
      var $extension = getFileExtension($nombre.value);
      switch ($extension) {
          case "png":
